...
| Workload narratives | Small demos show how preserved provenance affects LoRA/adapters, embedding projection, Conv1d, Torch FX capture, recovery, planning, and timing narratives. | `examples/fixed_weight_inference_demo.py`; `examples/torch_fx_frontend_demo.py`; `examples/adapter_workload_demo.py`; `examples/conv1d_workload_demo.py`; `examples/torch_coverage_demo.py` | Attention, masked attention, quantized modules, and CNN-block breadth are planned paper case-study targets until implemented with tests and artifacts. |
