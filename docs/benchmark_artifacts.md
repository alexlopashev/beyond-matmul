...
`docs/live_conv1d_benchmark_contract.md` defines the next live model-layer
Conv1d benchmark target before implementation. It selects
`openai/whisper-tiny` at revision
`169d4a4341b33bc18d8881c4b69c2e104e1cc0af`, layer
`model.encoder.conv1`, the Hugging Face Whisper widget LibriSpeech sample, and
an exact dense materialized Toeplitz fallback for required prefixes of 8, 16,
and 32 log-Mel frames.

The planned real artifact path is `docs/results/live_conv1d_whisper.json`,
generated by the future `benchmarks/live_conv1d_whisper.py` harness. Until that
harness and artifact land, this contract is only a design boundary: it does not
support a whitepaper speedup or memory-saving claim.
